...
# Created: 2024-07-23
# Description: Remove all access from archived repositories.
...
for repository in $(cat archived_repos_$organization.txt); do
  ...
  gh api /repos/$organization/$repository/teams > teams_$organization_$repository.json

  for team in $(jq -r '.[].slug' teams_$organization_$repository.json); do
  ...
  gh api /repos/$organization/$repository/collaborators?affiliation=direct --paginate > collaborators_$organization_$repository.json

  for collaborator in $(jq -r '.[].login' collaborators_$organization_$repository.json); do
